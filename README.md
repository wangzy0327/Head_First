...
- **创建型（5种）** ：[工厂模式](#工厂模式（factory）)、[抽象工厂模式](#抽象工厂（abstract factory）)、[单例模式](#单例模式（singleton）)、[原型模式](#原型模式)、[构建者模式](#构建者模式（生成器模式 Builder Pattern）)

- **结构型（7种）**：[适配器模式](#适配者模式)、[装饰器模式](#装饰器模式)、[代理模式](#代理模式)、[外观模式](#外观模式)、[桥接模式](#桥接模式)、[组合模式](#组合模式)、[享元模式](#享元模式（蝇量模式 Flyweight Pattern）)

- **行为型（11种）**：[模板方法模式](#模板方法模式)、[策略模式](#策略模式)、[观察者模式](#观察者模式（Observer）)、[中介者模式](#中介者模式)、[状态模式](#状态模式)、[责任链模式](#责任链模式)、[命令模式](#命令模式)、[迭代器模式](#迭代器模式)、[访问者模式](#访问者模式)、[解释器模式](#解释器模式)、[备忘录模式](#备忘录模式)
